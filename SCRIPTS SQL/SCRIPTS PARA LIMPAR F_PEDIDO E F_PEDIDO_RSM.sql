...
--SCRIPT SQL PARA LIMPAR F_PEDIDO_RSM (TRUNCAR OU DELETAR CONFORME VARIÁVEL TP_CARGA)

-- Quando o primeiro parâmetro (?) for igual a 'F', executa TRUNCATE
IF EXISTS (SELECT 1 WHERE ? = 'F')
    TRUNCATE TABLE F_PEDIDO_RSM;
ELSE
    -- Caso contrário, executa DELETE com base no segundo parâmetro (DT_REFERENCIA)
    DELETE FROM F_PEDIDO_RSM
    WHERE EXISTS (
        SELECT 1 
        FROM D_DATA
        WHERE F_PEDIDO_RSM.SK_MES_PEDIDO = D_DATA.SK_DATA
        AND D_DATA.DT_REFERENCIA >= ?
    );