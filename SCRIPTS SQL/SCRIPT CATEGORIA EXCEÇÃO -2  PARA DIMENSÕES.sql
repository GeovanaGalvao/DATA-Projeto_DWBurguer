--SCRIPT SQL PARA POPULAR CATEGORIA DE EXCEÇÃO -2 NÃO INFORMADO
...
DECLARE @ValorPadrao NVARCHAR(50) = 'NÃO INFORMADO';
DECLARE @ValorChaveNegativa INT = -2;
DECLARE @ValorAbv NVARCHAR(10) = 'N/I';
DECLARE @ValorChar2 CHAR(2) = '-2';
DECLARE @ValorChar1 CHAR(1) = 'X';
DECLARE @ValorNumericoPadrao INT = 0;

DECLARE DimCursor CURSOR FOR SELECT DimTable FROM @DimTables;
OPEN DimCursor;
...
BEGIN
    ...
    BEGIN
        SET @ExistsCheck = 'SELECT @RecordExists = COUNT(1) FROM ' 
                         + QUOTENAME(@DimTable) 
                         + ' WHERE ' + QUOTENAME(@SKColumn) + ' = ' 
                         + CAST(@ValorChaveNegativa AS NVARCHAR);
                         
        EXEC sp_executesql @ExistsCheck, N'@RecordExists INT OUTPUT', @RecordExists OUTPUT;
    END
