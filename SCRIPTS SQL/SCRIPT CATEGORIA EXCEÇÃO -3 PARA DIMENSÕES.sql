--SCRIPT SQL PARA POPULAR CATEGORIA DE EXCEÇÃO -3 NÃO ENCONTRADO
...
DECLARE @ValorPadrao NVARCHAR(50) = 'NÃO ENCONTRADO';
DECLARE @ValorChaveNegativa INT = -3;
DECLARE @ValorAbv NVARCHAR(10) = 'N/E';
DECLARE @ValorChar2 CHAR(2) = '-3';
...
BEGIN
    ...
    WHERE TABLE_NAME = @DimTable AND COLUMN_NAME LIKE 'SK_%';
    ...
    BEGIN
        ...
        SELECT 
            ...
            @Values = STRING_AGG(
                CASE 
                    ...
                    WHEN DATA_TYPE IN ('char', 'varchar') AND CHARACTER_MAXIMUM_LENGTH >= 14 THEN '''' + @ValorPadrao + ''''
                    WHEN DATA_TYPE IN ('char', 'varchar') AND CHARACTER_MAXIMUM_LENGTH < 14 THEN '''' + @ValorAbv + ''''
                    WHEN DATA_TYPE IN ('date', 'datetime', 'datetime2') THEN 
                        CASE WHEN COLUMN_NAME IN( 'DT_CRI_RGT', 'DT_INICIO', 'DT_NASCIMENTO', 'DT_CADASTRAMENTO') THEN 'GETDATE()' ELSE 'NULL' END
        ...
        SET @SQL = @SQL 
                 + 'INSERT INTO ' + QUOTENAME(@DimTable) + ' (' + @Columns + ') VALUES (' + @Values + ');';
        SET @SQL = @SQL 
                 + ' SET IDENTITY_INSERT ' + QUOTENAME(@DimTable) + ' OFF;';
